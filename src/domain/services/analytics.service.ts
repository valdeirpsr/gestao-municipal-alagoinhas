...
export class AnalyticsService implements IAnalyticsService {
  ...
  getTotalBudget(): number {
    ...
    return contracts
      .map((contract) => (contract.receita ? contract.valorGlobal * -1 : contract.valorGlobal))
  ...
  getTotalExpenses(): number {
    const contracts = this.contractRepository.getAll();
    return contracts.filter((contract) => contract.receita === false).reduce((acc, curr) => acc + curr.valorGlobal, 0);
  }
  ...
  getTotalRevenues(): number {
    const contracts = this.contractRepository.getAll();
    return contracts.filter((contract) => contract.receita === true).reduce((acc, curr) => acc + curr.valorGlobal, 0);
  }
  ...
  getTopAgencies(limit: number): AgencyAnalysis[] {
    ...
    return _(contracts)
      .groupBy("unidadeOrgao.nomeUnidade")
      .map((contract, key) => ({
